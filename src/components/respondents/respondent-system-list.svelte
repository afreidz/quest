...
>
  <table class="table table-pin-rows">
    {#each respondent.revisions as revision}
      <thead>
        <tr class="bg-base-100/15 border-neutral-300">
          <td class="p-4 text-sm" colspan="2">
            <div class="flex justify-between">
              {revision.title}
              <i class="badge not-italic badge-primary"
                >{revision.system.client.name}</i
              >
            </div>
          </td>
        </tr>
        ...
        <tr class="bg-neutral border-neutral-200">
          ...
          <td class="text-right">
            <aside class="mr-4 inline-flex gap-4">
              {#if hasCompletedSurvey(revision.survey?.id)}
                <span class="badge badge-primary">completed survey</span>
              {/if}
              {#if hasCompletedSurvey(revision.checklist?.id)}
